"""
 Title:         Multi-temperature theta projection model
...
class Model(__Model__):
    ...
    def initialise(self, **settings):
        ...
        # t0 = exp(p0[1] + p0[2]*x1 + p0[3]*x2 + p0[4])*x1*x2;
        # t1 = exp(p1[1] + p1[2]*x1 + p1[3]*x2 + p1[4])*x1*x2;
        # t2 = exp(p2[1] + p2[2]*x1 + p2[3]*x2 + p2[4])*x1*x2;
        # t3 = exp(p3[1] + p3[2]*x1 + p3[3]*x2 + p3[4])*x1*x2;
        # t4 = p4[1]*x2^p4[2]*exp(-p4[3]/8.3145/x2);
        self.kr_combine = """
            t0 = abs(p[1])/y1;
            t1 = abs(p[2]);
            t2 = abs(p[3])/y1;
            t3 = abs(p[4]);
            t4 = abs(p[5])/y1;
            z0 = t0*(1-exp(-t1*x0)) + t2*(exp(-t3*x0)-1) + t4*x0;
            z1 = f1(x1,x2);
        ...
        kr_spec = TemplateExpressionSpec(
            ...
            parameters     = {"p": 5},
            # parameters     = {"p0": 4, "p1": 4, "p2": 4, "p3": 4, "p4": 3},
    ...
    def get_latex(self) -> str:
        ...
        latex_expressions = [latex_dict[p] for p in ["z0", "z1", "t0", "t1", "t2", "t3", "t4"]]
